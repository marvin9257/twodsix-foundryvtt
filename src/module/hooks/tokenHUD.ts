...
class TwodsixActionHandler extends ActionHandler {
  ...
  async buildSystemActions(character, subcategoryIds) {
    ...
    this._getCharacteristics    (actor, tokenId, { id: CHARACTERISTICS, type: 'system' });
    this._getSkills   (actor, tokenId, { id: SKILLS_ID, type: 'system' });
    this._getWeapons   (actor, tokenId, { id: WEAPON_ID, type: 'system' });
    this._getEquipment(actor, tokenId, { id: EQUIPMENT_ID, type: 'system' });

    //if (settings.get("showHudTitle")) result.hudTitle = token.name;
  }

  _getCharacteristics(actor, tokenId, idType) {
    ...
    let actions = [ "might", "speed", "intellect" ].map( key => {
      return {
        ...
        encodedValue: [CHARACTERISTICS, actor.id, tokenId, key.capitalize()].join(this.delimiter)
      };
    });
    /*
        ...
        */
    this.addActionsToActionList(actions, idType);
  }

  _getWeapons(actor, tokenId, parent) {
...
export class MySystemManager extends SystemManager {
  ...
  doGetActionHandler (categoryManager) {
    return new TwodsixActionHandler(categoryManager);
  }
